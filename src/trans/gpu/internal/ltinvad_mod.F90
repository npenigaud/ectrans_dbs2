...
CONTAINS
  ...
  SUBROUTINE LTINVAD(ALLOCATOR,HLTINVAD,KF_UV,KF_SCALARS,&
     ...
     & ZOUTS,ZOUTA,ZOUTS0,ZOUTA0,LDCOMPUTE)
...
!    CALL C_F_POINTER(C_LOC(PIA_L), PIA, (/ 2*IF_READIN, R%NTMAX+3, D%NUMP /))
    ...
    CALL MPL_DOUBLESTACK%ALLOCATE(PIA,[2*IF_READIN, R%NTMAX+3, D%NUMP])
...
!        & IALLOC_POS, IALLOC_SZ)
    CALL MPL_DOUBLESTACK%ALLOCATE(ZINP,[IIN_SIZE])
    IALLOC_POS = IALLOC_POS + IALLOC_SZ
...
!        & IALLOC_POS, IALLOC_SZ)
    CALL MPL_DOUBLESTACK%ALLOCATE(ZINP0,[IIN0_SIZE])
    IALLOC_POS = IALLOC_POS + IALLOC_SZ
IF (LLCOMPUTE) THEN
