...
CONTAINS
  FUNCTION PREPARE_LTINVAD(ALLOCATOR,KF_UV,KF_SCALARS,LVORGP,LDIVGP,LSCDERS) RESULT(HLTINVAD)
  ...
  SUBROUTINE LTINVAD(ALLOCATOR,HLTINVAD,KF_UV,KF_SCALARS,&
    ...
    REAL(KIND=JPRBT)  , POINTER, INTENT(INOUT) :: ZOUTS(:), ZOUTA(:)
    REAL(KIND=JPRD)   , POINTER, INTENT(INOUT) :: ZOUTS0(:), ZOUTA0(:)
...
!    CALL ASSIGN_PTR(PIA_L, GET_ALLOCATION(ALLOCATOR, HLTINVAD%HPIA_AND_IN),&
!        & IALLOC_POS, IALLOC_SZ)
!    CALL C_F_POINTER(C_LOC(PIA_L), PIA, (/ 2*IF_READIN, R%NTMAX+3, D%NUMP /))
    CALL MPL_DOUBLESTACK%NEXT_SIDE()
    CALL MPL_DOUBLESTACK%ALLOCATE(PIA,[2*IF_READIN, R%NTMAX+3, D%NUMP],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE)
    IALLOC_POS = IALLOC_POS + IALLOC_SZ


    ! ZINP
    IALLOC_SZ = ALIGN(IIN_SIZE*C_SIZEOF(ZINP(1)),128)
!    CALL ASSIGN_PTR(ZINP, GET_ALLOCATION(ALLOCATOR, HLTINVAD%HPIA_AND_IN),&
!        & IALLOC_POS, IALLOC_SZ)
    CALL MPL_DOUBLESTACK%ALLOCATE(ZINP,[IIN_SIZE],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE)
    IALLOC_POS = IALLOC_POS + IALLOC_SZ

    ! ZINP0
    IALLOC_SZ = ALIGN(IIN0_SIZE*C_SIZEOF(ZINP0(1)),128)
!    CALL ASSIGN_PTR(ZINP0, GET_ALLOCATION(ALLOCATOR, HLTINVAD%HPIA_AND_IN),&
!        & IALLOC_POS, IALLOC_SZ)
    CALL MPL_DOUBLESTACK%ALLOCATE(ZINP0,[IIN0_SIZE],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE)
    IALLOC_POS = IALLOC_POS + IALLOC_SZ
...
#ifdef OMPGPU
!$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(2) DEFAULT(NONE) SHARED(D,PIA_LEDIR,IF_LEG) PRIVATE(KM) MAP(TO:IF_LEG)
#endif
#ifdef ACCGPU
    !$ACC DATA PRESENT(D,D_MYMS,D_NUMP,PIA_LEDIR)
    ...
    !*       3.    Adjoint of SPECTRAL COMPUTATIONS FOR U,V AND DERIVATIVES.
    !              ----------------------------------------------
    ...
    ENDIF
    CALL GSTATS(422,1)
    CALL MPL_DOUBLESTACK%DEALLOCATE(ZINP0)    
    CALL MPL_DOUBLESTACK%DEALLOCATE(ZINP)
    CALL MPL_DOUBLESTACK%DEALLOCATE(PIA)
    CALL MPL_DOUBLESTACK%DEALLOCATE(ZOUTS0)
    CALL MPL_DOUBLESTACK%DEALLOCATE(ZOUTA0)
    CALL MPL_DOUBLESTACK%DEALLOCATE(ZOUTS)
    CALL MPL_DOUBLESTACK%DEALLOCATE(ZOUTA)
