  ...
  SUBROUTINE DIR_TRANS_CTLAD(KF_UV_G,KF_SCALARS_G,KF_GP,KF_FS,KF_UV,KF_SCALARS,&
    ...
    IF (KF_FS > 0) THEN
      CALL LTDIRAD(ALLOCATOR,HLTDIR,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV,KF_SCALARS, &
                 ...
                 & PSPSC3A,PSPSC3B,PSPSC2,LDCOMPUTE=LLCOMPUTE)

      CALL GSTATS(153,0)
      CALL TRLTOMAD_UNPACK(ALLOCATOR,HTRLTOM_UNPACK,FOUBUF,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV,LDCOMPUTE=LLCOMPUTE)
      CALL TRLTOMAD(ALLOCATOR,HTRLTOM,FOUBUF_IN,FOUBUF,KF_FS,LDCOMPUTE=LLCOMPUTE)
      CALL TRLTOMAD_PACK(ALLOCATOR,HTRLTOM_PACK,PREEL_COMPLEX,FOUBUF_IN,KF_FS,LDCOMPUTE=LLCOMPUTE)
      CALL GSTATS(153,1)

      ! fourier transform from PREEL_REAL to PREEL_COMPLEX (in-place!)
      CALL GSTATS(1640,0)
      CALL FTINV(ALLOCATOR,HFTINV,PREEL_COMPLEX,PREEL_REAL,KF_FS,LDCOMPUTE=LLCOMPUTE)
      CALL GSTATS(1640,1)
    ENDIF
...
!    IF (ASSOCIATED(ZINPA0)) CALL MPL_DOUBLESTACK%DEALLOCATE(ZINPA0)
!    IF (ASSOCIATED(ZINPS0)) CALL MPL_DOUBLESTACK%DEALLOCATE(ZINPS0)
!    IF (ASSOCIATED(ZINPA)) CALL MPL_DOUBLESTACK%DEALLOCATE(ZINPA)
!    IF (ASSOCIATED(ZINPS)) CALL MPL_DOUBLESTACK%DEALLOCATE(ZINPS)
