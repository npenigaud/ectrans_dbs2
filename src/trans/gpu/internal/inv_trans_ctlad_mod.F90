...
MODULE INV_TRANS_CTLAD_MOD
  CONTAINS
    SUBROUTINE INV_TRANS_CTLAD(KF_UV_G,KF_SCALARS_G,KF_GP,KF_FS,KF_OUT_LT,&
      ...
      & PSPSC3A,PSPSC3B,PSPSC2,KVSETSC3A,KVSETSC3B,KVSETSC2,PGPUV,PGP3A,PGP3B,PGP2,LDCOMPUTE)
      ...
      USE MPL_DOUBLESTACK_MOD,    ONLY : MPL_DOUBLESTACK, MPL_LOWER, MPL_UPPER 
      IMPLICIT NONE
  
      ! Declaration of arguments
      LOGICAL, INTENT(IN), OPTIONAL :: LDCOMPUTE 
...
!!      ENDIF
     ...
     CALL MPL_DOUBLESTACK%SET_SIDE(MPL_LOWER)
     IF (.NOT. LLCOMPUTE) CALL MPL_DOUBLESTACK%INIT_MEMORY_MEASUREMENT()
      ...
      CALL TRGTOL(ALLOCATOR,HTRGTOL,PREEL_REAL,IF_FOURIER,KF_GP,KF_UV_G,KF_SCALARS_G,&
       ...
       & PGP=PGP,PGPUV=PGPUV,PGP3A=PGP3A,PGP3B=PGP3B,PGP2=PGP2,LDCOMPUTE=LLCOMPUTE)
      CALL GSTATS(157,1)
  
      IF (KF_FS > 0) THEN
        CALL GSTATS(107,0)
        ! Fourier transformations
        CALL FTDIR(ALLOCATOR,HFTDIR,PREEL_REAL,PREEL_COMPLEX,IF_FOURIER,LDCOMPUTE=LLCOMPUTE)
        ! compute NS derivatives
        IF (LLCOMPUTE) CALL FSCAD(PREEL_COMPLEX, IF_FOURIER, KF_UV, KF_SCALARS, KUV_OFFSET, KSCALARS_OFFSET, &
        & KSCALARS_NSDER_OFFSET, KUV_EWDER_OFFSET, KSCALARS_EWDER_OFFSET)
        CALL GSTATS(107,1)
        
        ! Packing into send buffer, to fourier space and unpack
        CALL GSTATS(152,0)
        CALL TRMTOLAD_UNPACK(ALLOCATOR,HTRMTOL_UNPACK,FOUBUF,PREEL_COMPLEX,IF_LEG,IF_FOURIER,LDCOMPUTE=LLCOMPUTE)
        CALL TRMTOLAD(ALLOCATOR,HTRMTOL,FOUBUF_IN,FOUBUF,IF_LEG,LDCOMPUTE=LLCOMPUTE)
        CALL TRMTOLAD_PACK(ALLOCATOR,HTRMTOL_PACK,ZOUTS,ZOUTA,ZOUTS0,ZOUTA0,FOUBUF_IN,IF_LEG,LDCOMPUTE=LLCOMPUTE)
        CALL GSTATS(152,1)
        ...
        CALL LTINVAD(ALLOCATOR,HLTINVAD,KF_UV,KF_SCALARS,&
            ...
            & ZOUTS,ZOUTA,ZOUTS0,ZOUTA0,LDCOMPUTE=LLCOMPUTE)
        CALL GSTATS(102,1)
      ENDIF
!      IF (ASSOCIATED(PREEL_REAL)) CALL MPL_DOUBLESTACK%DEALLOCATE(PREEL_REAL)
!      IF (ASSOCIATED(PREEL_COMPLEX)) CALL MPL_DOUBLESTACK%DEALLOCATE(PREEL_COMPLEX)
!      IF (ASSOCIATED(FOUBUF_IN)) CALL MPL_DOUBLESTACK%DEALLOCATE(FOUBUF_IN)
!      IF (ASSOCIATED(FOUBUF)) CALL MPL_DOUBLESTACK%DEALLOCATE(FOUBUF)
!      IF (ASSOCIATED(ZOUTA0)) CALL MPL_DOUBLESTACK%DEALLOCATE(ZOUTA0)
!      IF (ASSOCIATED(ZOUTS0)) CALL MPL_DOUBLESTACK%DEALLOCATE(ZOUTS0)
!      IF (ASSOCIATED(ZOUTA)) CALL MPL_DOUBLESTACK%DEALLOCATE(ZOUTA)
!      IF (ASSOCIATED(ZOUTS)) CALL MPL_DOUBLESTACK%DEALLOCATE(ZOUTS)

   IF (.NOT. LLCOMPUTE) CALL MPL_DOUBLESTACK%END_MEMORY_MEASUREMENT()      
