...
CONTAINS
  ...
  SUBROUTINE TRMTOL(ALLOCATOR,HTRMTOL,PFBUF_IN,PFBUF,KF_LEG,LDCOMPUTE)
...
!        & 1_JPIB, 2_JPIB*D%NLENGT0B*KF_LEG*C_SIZEOF(PFBUF(1)))
    ...
    CALL MPL_DOUBLESTACK%ALLOCATE(PFBUF,[2_JPIB*D%NLENGT0B*KF_LEG],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
IF (LLCOMPUTE) THEN
    IF(NPROC > 1) THEN
      DO J=1,NPRTRW
    ...
    ELSE
      ...
      CALL GSTATS(1608,1)
    ENDIF
    CALL MPL_DOUBLESTACK%DEALLOCATE(PFBUF_IN)
ENDIF
