...
CONTAINS
  ...
  END FUNCTION
  SUBROUTINE TRMTOL_PACK(ALLOCATOR,HTRMTOL_PACK,ZOUTS,ZOUTA,ZOUTS0,ZOUTA0,FOUBUF_IN,KF_LEG,LDCOMPUTE)
    ...
    REAL(KIND=JPRBT), INTENT(INOUT), POINTER :: ZOUTS(:), ZOUTA(:)
    REAL(KIND=JPRD), INTENT(INOUT), POINTER :: ZOUTS0(:), ZOUTA0(:)
    INTEGER(KIND=JPIM), INTENT(IN)  :: KF_LEG
...
!      & 1_JPIB, 2_JPIB*D%NLENGT1B*KF_LEG*C_SIZEOF(FOUBUF_IN(1)))
    ...
    CALL MPL_DOUBLESTACK%ALLOCATE(FOUBUF_IN,[2_JPIB*D%NLENGT1B*KF_LEG],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
...
SUBROUTINE TRMTOL_UNPACK(ALLOCATOR,HTRMTOL_UNPACK,FOUBUF,PREEL_COMPLEX,KF_CURRENT,KF_TOTAL,LDCOMPUTE)
...
CALL MPL_DOUBLESTACK%NEXT_SIDE()
CALL MPL_DOUBLESTACK%ALLOCATE(PREEL_COMPLEX,[1_JPIB*KF_TOTAL*D%NLENGTF],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
IF (LLCOMPUTE) THEN
#ifdef OMPGPU
...
!$ACC PARALLEL LOOP PRIVATE(IGLG,IOFF_LAT,ISTA,RET_REAL,RET_COMPLEX) FIRSTPRIVATE(KF_CURRENT,&
!$ACC&              KF_TOTAL,OFFSET_VAR,ILOEN_MAX) DEFAULT(NONE) COLLAPSE(3) &
#ifndef _CRAYFTN
!$ACC& ASYNC(1)
#else
!$ACC&
