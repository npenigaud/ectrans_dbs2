...
CONTAINS
  ...
  SUBROUTINE FTDIR(ALLOCATOR,HFTDIR,PREEL_REAL,PREEL_COMPLEX,KFIELD,LDCOMPUTE)
    ...
    CALL MPL_DOUBLESTACK%ALLOCATE(PREEL_COMPLEX,[1_JPIB*KFIELD*D%NLENGTF],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
...
#endif
#ifdef IN_PLACE_FFT 
    !  preel_real is still used as an alias (by preel_complex) 
    NULLIFY(PREEL_REAL)
#else
   CALL MPL_DOUBLESTACK%DEALLOCATE(PREEL_REAL)
#endif
ENDIF
    END ASSOCIATE
