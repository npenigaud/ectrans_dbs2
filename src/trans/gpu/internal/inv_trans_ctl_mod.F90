...
!!    HTRLTOG = PREPARE_TRLTOG(ALLOCATOR,IF_FOURIER,KF_GP)
    ...
    CALL MPL_DOUBLESTACK%SET_SIDE(MPL_UPPER)
    ...
    IF (KF_FS > 0) THEN
      ...
      CALL GSTATS(152,0)
      CALL TRMTOL_PACK(ALLOCATOR,HTRMTOL_PACK,ZOUTS,ZOUTA,ZOUTS0,ZOUTA0,FOUBUF_IN,IF_LEG,LDCOMPUTE=LLCOMPUTE)
      CALL TRMTOL(ALLOCATOR,HTRMTOL,FOUBUF_IN,FOUBUF,IF_LEG,LDCOMPUTE=LLCOMPUTE)
      CALL TRMTOL_UNPACK(ALLOCATOR,HTRMTOL_UNPACK,FOUBUF,PREEL_COMPLEX,IF_LEG,IF_FOURIER,LDCOMPUTE=LLCOMPUTE)

      CALL GSTATS(152,1)

      CALL GSTATS(107,0)
      ! compute NS derivatives
      IF (LLCOMPUTE) CALL FSC(PREEL_COMPLEX, IF_FOURIER, KF_UV, KF_SCALARS, KUV_OFFSET, KSCALARS_OFFSET, &
          & KSCALARS_NSDER_OFFSET, KUV_EWDER_OFFSET, KSCALARS_EWDER_OFFSET)
      !Legendre transformations
      CALL FTINV(ALLOCATOR, HFTINV, PREEL_COMPLEX,PREEL_REAL,IF_FOURIER,LDCOMPUTE=LLCOMPUTE)
      CALL GSTATS(107,1)
    ENDIF
