...
CONTAINS
  ...
  SUBROUTINE TRLTOMAD(ALLOCATOR,HTRLTOMAD,PFBUF_IN,PFBUF,KF_FS,LDCOMPUTE)
...
!        & 1_JPIB, 2_JPIB*D%NLENGT0B*KF_FS*C_SIZEOF(PFBUF_IN(1)))
    ...
    CALL MPL_DOUBLESTACK%ALLOCATE(PFBUF_IN,[2_JPIB*D%NLENGT0B*KF_FS])
IF (LLCOMPUTE) THEN
    ...
    IF(NPROC > 1) THEN
      ...
      IF (ILENS(IRANK) > 0) THEN
          ...
          DO JPOS=FROM_RECV,TO_RECV
             PFBUF_IN(JPOS-FROM_RECV+FROM_SEND) = PFBUF(JPOS)
          ENDDO
    ...
    !$ACC END DATA
#endif
    CALL MPL_DOUBLESTACK%DEALLOCATE(PFBUF)
ENDIF
