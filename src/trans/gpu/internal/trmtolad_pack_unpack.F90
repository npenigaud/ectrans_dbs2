...
CONTAINS
  ...
  END FUNCTION
  SUBROUTINE TRMTOLAD_PACK(ALLOCATOR,HTRMTOL_PACK,ZOUTS,ZOUTA,ZOUTS0,ZOUTA0,FOUBUF_IN,KF_LEG,LDCOMPUTE)
    ...
    REAL(KIND=JPRB), INTENT(INOUT), POINTER :: FOUBUF_IN(:)
...
!        & IALLOC_POS, IALLOC_SZ)
  ...
  SUBROUTINE TRMTOLAD_UNPACK(ALLOCATOR,HTRMTOL_UNPACK,FOUBUF,PREEL_COMPLEX,KF_CURRENT,KF_TOTAL,LDCOMPUTE)
  ...
  REAL(KIND=JPRBT), INTENT(IN), POINTER :: PREEL_COMPLEX(:)
...
!                & 1_JPIB, 2_JPIB*D%NLENGT0B*KF_CURRENT*C_SIZEOF(FOUBUF(1)))
  CALL MPL_DOUBLESTACK%NEXT_SIDE
  CALL MPL_DOUBLESTACK%ALLOCATE(FOUBUF,[2_JPIB*D%NLENGT0B*KF_CURRENT]) 
IF (LLCOMPUTE) THEN
#ifdef OMPGPU
  ...
  !$OMP& PRIVATE(IGLG,IOFF_LAT,ISTA) &
  !$OMP& MAP(TO:KF_CURRENT,ILOEN_MAX,OFFSET_VAR,KF_TOTAL)
#endif
#ifdef ACCGPU
!  !$ACC PARALLEL LOOP PRIVATE(IGLG,IOFF_LAT,ISTA) FIRSTPRIVATE(KF_CURRENT,&
