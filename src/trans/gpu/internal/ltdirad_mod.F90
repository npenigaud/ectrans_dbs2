...
!    CALL C_F_POINTER(C_LOC(POA1_L), POA1, (/ 2*KF_FS, R%NTMAX+3, D%NUMP /))
    ...
    CALL MPL_DOUBLESTACK%ALLOCATE(POA1,[2*KF_FS, R%NTMAX+3, D%NUMP],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE)
...
!    CALL C_F_POINTER(C_LOC(POA2_L), POA2, (/ 4*KF_UV, R%NTMAX+3, D%NUMP /))
    CALL MPL_DOUBLESTACK%ALLOCATE(POA2,[4*KF_UV, R%NTMAX+3, D%NUMP],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE)
    IALLOC_POS = IALLOC_POS + IALLOC_SZ
...
!        & IALLOC_POS, IALLOC_SZ, SET_STREAM=1)
    CALL MPL_DOUBLESTACK%ALLOCATE(ZOUT,[IOUT_SIZE],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE)
    IALLOC_POS = IALLOC_POS + IALLOC_SZ
...
!        & IALLOC_POS, IALLOC_SZ, SET_STREAM=1)
    ...
    CALL MPL_DOUBLESTACK%ALLOCATE(ZOUT0,[IOUT0_SIZE],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE)
    IALLOC_POS = IALLOC_POS + IALLOC_SZ
...
!        & IALLOC_POS, IALLOC_SZ)
    CALL MPL_DOUBLESTACK%NEXT_SIDE()
    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPS,[IIN_SIZE],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE)
    IALLOC_POS=IALLOC_POS+IALLOC_SZ
    IALLOC_SZ = ALIGN(IIN_SIZE*C_SIZEOF(ZINPA(0)),128)
!    CALL ASSIGN_PTR(ZINPA, GET_ALLOCATION(ALLOCATOR, HLTDIR%HINPS_AND_ZINPA),&
!        & IALLOC_POS, IALLOC_SZ)
    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPA,[IIN_SIZE],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE)
    IALLOC_POS=IALLOC_POS+IALLOC_SZ

    IALLOC_SZ = ALIGN(IIN0_SIZE*C_SIZEOF(ZINPS0(0)),128)
!    CALL ASSIGN_PTR(ZINPS0, GET_ALLOCATION(ALLOCATOR, HLTDIR%HINPS_AND_ZINPA),&
!        & IALLOC_POS, IALLOC_SZ)
    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPS0,[IIN0_SIZE],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE)
    IALLOC_POS=IALLOC_POS+IALLOC_SZ

    IALLOC_SZ = ALIGN(IIN0_SIZE*C_SIZEOF(ZINPA0(0)),128)
!    CALL ASSIGN_PTR(ZINPA0, GET_ALLOCATION(ALLOCATOR, HLTDIR%HINPS_AND_ZINPA),&
!        & IALLOC_POS, IALLOC_SZ)
    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPA0,[IIN0_SIZE],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE)
    IALLOC_POS=IALLOC_POS+IALLOC_SZ

#ifdef ACCGPU
    !$ACC DATA PRESENT(POA1,R,R_NTMAX,D,D_NUMP) 
    !$ACC PARALLEL LOOP COLLAPSE(3) DEFAULT(NONE) COPYIN(KF_FS)
#endif
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(PRESENT,ALLOC:POA1,R_NTMAX,D_NUMP)
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO COLLAPSE(3) DEFAULT(NONE) SHARED(R,D,KF_FS,POA1) &
    !$OMP& MAP(TO:KF_FS)
#endif
    DO KMLOC=1,D_NUMP
        DO JN=1,R_NTMAX+3
    ...
    !     ------------------------------------------------------------------
    CALL MPL_DOUBLESTACK%DEALLOCATE(ZOUT0)    
    CALL MPL_DOUBLESTACK%DEALLOCATE(ZOUT)
    CALL MPL_DOUBLESTACK%DEALLOCATE(POA2)
    CALL MPL_DOUBLESTACK%DEALLOCATE(POA1)
