...
!    CALL C_F_POINTER(C_LOC(POA1_L), POA1, (/ 2*KF_FS, R%NTMAX+3, D%NUMP /))
    ...
    CALL MPL_DOUBLESTACK%ALLOCATE(POA1,[2*KF_FS, R%NTMAX+3, D%NUMP])
...
!    CALL C_F_POINTER(C_LOC(POA2_L), POA2, (/ 4*KF_UV, R%NTMAX+3, D%NUMP /))
    CALL MPL_DOUBLESTACK%ALLOCATE(POA2,[4*KF_UV, R%NTMAX+3, D%NUMP])
    IALLOC_POS = IALLOC_POS + IALLOC_SZ
...
!        & IALLOC_POS, IALLOC_SZ, SET_STREAM=1)
    CALL MPL_DOUBLESTACK%ALLOCATE(ZOUT,[IOUT_SIZE])
    IALLOC_POS = IALLOC_POS + IALLOC_SZ
...
!        & IALLOC_POS, IALLOC_SZ, SET_STREAM=1)
    ...
    CALL MPL_DOUBLESTACK%ALLOCATE(ZOUT0,[IOUT0_SIZE])
    IALLOC_POS = IALLOC_POS + IALLOC_SZ
