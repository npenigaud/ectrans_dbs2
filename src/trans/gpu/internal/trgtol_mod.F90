...
CONTAINS
  ...
  SUBROUTINE TRGTOL(ALLOCATOR,HTRGTOL,PREEL_REAL,KF_FS,KF_GP,KF_UV_G,KF_SCALARS_G,&
     &PGP,PGPUV,PGP3A,PGP3B,PGP2,KPTRGP,KVSETUV,KVSETSC,KVSETSC3A,KVSETSC3B,KVSETSC2,LDCOMPUTE)
...
#define TRGTOL_DTYPE MPI_REAL8
    ...
    CALL MPL_DOUBLESTACK%ALLOCATE(PREEL_REAL,[1_JPIB*KF_FS*D%NLENGTF],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
...
!ov!!      CALL ASSIGN_PTR(ZCOMBUFS, GET_ALLOCATION(ALLOCATOR, HTRGTOL%HCOMBUFS),&
!ov!!          & 1_JPIB, ICOMBUFS_OFFSET(ISEND_COUNTS+1)*C_SIZEOF(ZCOMBUFS(1)))
!ov!      CALL MPL_DOUBLESTACK%ALLOCATE(ZCOMBUFS,[ICOMBUFS_OFFSET(ISEND_COUNTS+1)],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE)
!ov!    ENDIF
...
!ov!    CALL GSTATS(411,0)
!ov!    CALL MPL_DOUBLESTACK%NEXT_SIDE()
!ov!    IF (IRECV_COUNTS > 0) THEN
!ov!!      CALL ASSIGN_PTR(ZCOMBUFR, GET_ALLOCATION(ALLOCATOR, HTRGTOL%HCOMBUFR_AND_REEL),&
!ov!!          & 1_JPIB, ICOMBUFR_OFFSET(IRECV_COUNTS+1)*C_SIZEOF(ZCOMBUFR(1)))
!ov!      CALL MPL_DOUBLESTACK%ALLOCATE(ZCOMBUFR,[ICOMBUFR_OFFSET(IRECV_COUNTS+1)],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE)
!ov!    ENDIF
#ifdef OMPGPU
