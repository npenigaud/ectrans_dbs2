...
CONTAINS
  ...
  SUBROUTINE FTINV(ALLOCATOR,HFTINV,PREEL_COMPLEX,PREEL_REAL,KFIELD,LDCOMPUTE)
...
!!      & 1_JPIB, 1_JPIB*KFIELD*D%NLENGTF*C_SIZEOF(PREEL_REAL(1)))
    ...
    CALL MPL_DOUBLESTACK%ALLOCATE(PREEL_REAL,[1_JPIB*KFIELD*D%NLENGTF],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
...
#endif
#ifdef IN_PLACE_FFT
!   preel_complex was not directly allocated, will be deallocated when preel_real is
!    CALL MPL_DOUBLESTACK%DEALLOCATE(PREEL_COMPLEX)
