...
#ifdef IN_PLACE_FFT
    write (0,*) "AFFICHAGE in_place_fft inv"
    call flush(0)
    PREEL_REAL => PREEL_COMPLEX
#else
     write (0,*) "AFFICHAGE out_of_place_fft inv"
    call flush(0)
...
!!      & 1_JPIB, 1_JPIB*KFIELD*D%NLENGTF*C_SIZEOF(PREEL_REAL(1)))
    ...
    CALL MPL_DOUBLESTACK%ALLOCATE(PREEL_REAL,[1_JPIB*KFIELD*D%NLENGTF],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE)
#endif
