...
#define TRLTOG_DTYPE MPI_REAL8
    ...
    !ZCOMBUFR and ZCOMBUFS are allocated on the same side
...
!      CALL MPL_DOUBLESTACK%ALLOCATE(ZCOMBUFR,[ICOMBUFR_OFFSET(IRECV_COUNTS+1)],SIDE_PARAM=MPL_UPPER)
      CALL MPL_DOUBLESTACK%ALLOCATE(ZCOMBUFR,[ICOMBUFR_OFFSET(IRECV_COUNTS+1)],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
...
!      CALL MPL_DOUBLESTACK%ALLOCATE(ZCOMBUFS,[ICOMBUFS_OFFSET(ISEND_COUNTS+1)],SIDE_PARAM=MPL_UPPER)

      CALL MPL_DOUBLESTACK%ALLOCATE(ZCOMBUFS,[ICOMBUFS_OFFSET(ISEND_COUNTS+1)],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
...
!ov!    CALL GSTATS(1806,1)
!ov!    
!ov!    ! Figure out processes that send or recv something
!ov!    ISEND_COUNTS   = 0
!ov!    IRECV_COUNTS   = 0
!ov!    DO JROC=1,NPROC
!ov!      IF( JROC /= MYPROC) THEN
!ov!        IF(IRECVTOT(JROC) > 0) THEN
!ov!          ! I have to recv something, so let me store that
!ov!          IRECV_COUNTS = IRECV_COUNTS + 1
!ov!          IRECV_TO_PROC(IRECV_COUNTS)=JROC
!ov!        ENDIF
!ov!        IF(ISENDTOT(JROC) > 0) THEN
!ov!          ! I have to send something, so let me store that
!ov!          ISEND_COUNTS = ISEND_COUNTS+1
!ov!          ISEND_TO_PROC(ISEND_COUNTS)=JROC
!ov!        ENDIF
!ov!      ENDIF
!ov!    ENDDO
!ov!
!ov!    ! ... build this data structure now during the MPI communication
!ov!    ! Allocate this buffer now. Add 1 for self contribution
!ov!    ALLOCATE(IFLDA(KF_GP,1+IRECV_COUNTS))
!ov!
!ov!    ! Copy local contribution
!ov!    IF(LLOCAL_CONTRIBUTION) THEN
!ov!      ! I have to send something to myself...
!ov!
!ov!      ! Input is KF_GP fields. We find the resulting KF_FS fields.
!ov!      IFLDS = 0
!ov!      DO JFLD=1,KF_GP
!ov!        IF(IVSET(JFLD) == MYSETV .OR. IVSET(JFLD) == -1) THEN
!ov!          IFLDS = IFLDS+1
!ov!          IF(PRESENT(KPTRGP)) THEN
!ov!            IFLDA(IFLDS,1) = KPTRGP(JFLD)
!ov!          ELSE
!ov!            IFLDA(IFLDS,1) = JFLD
!ov!          ENDIF
!ov!        ENDIF
!ov!      ENDDO
!ov!    ENDIF
!ov!
!ov!    DO INR=1,IRECV_COUNTS
!ov!      IRECV=IRECV_TO_PROC(INR)
!ov!      CALL PE2SET(IRECV,ISETA,ISETB,ISETW,ISETV)
!ov!      IFLDS = 0
!ov!      DO JFLD=1,KF_GP
!ov!        IF(IVSET(JFLD) == ISETV .OR. IVSET(JFLD) == -1 ) THEN
!ov!          IFLDS = IFLDS+1
!ov!          IF(PRESENT(KPTRGP)) THEN
!ov!            IFLDA(IFLDS,1+INR)=KPTRGP(JFLD)
!ov!          ELSE
!ov!            IFLDA(IFLDS,1+INR)=JFLD
!ov!          ENDIF
!ov!        ENDIF
!ov!      ENDDO
!ov!    ENDDO
...
!ov!    ALLOCATE(ICOMBUFS_OFFSET(ISEND_COUNTS+1))
!ov!    ICOMBUFS_OFFSET(1) = 0
!ov!    DO JROC=1,ISEND_COUNTS
!ov!      ICOMBUFS_OFFSET(JROC+1) = ICOMBUFS_OFFSET(JROC) + ISENDTOT(ISEND_TO_PROC(JROC))
!ov!    ENDDO
!ov!    ALLOCATE(ICOMBUFR_OFFSET(IRECV_COUNTS+1))
!ov!    ICOMBUFR_OFFSET(1) = 0
!ov!    DO JROC=1,IRECV_COUNTS
!ov!      ICOMBUFR_OFFSET(JROC+1) = ICOMBUFR_OFFSET(JROC) + IRECVTOT(IRECV_TO_PROC(JROC))
!ov!    ENDDO
!ov! 
!ov!    !ZCOMBUFR and ZCOMBUFS are allocated on the same side
!ov!    CALL MPL_DOUBLESTACK%NEXT_SIDE()
!ov!    IF (IRECV_COUNTS > 0) THEN
!ov!!      CALL ASSIGN_PTR(ZCOMBUFR, GET_ALLOCATION(ALLOCATOR, HTRLTOG%HCOMBUFR_AND_COMBUFS),&
!ov!!          & 1_JPIB, ICOMBUFR_OFFSET(IRECV_COUNTS+1)*C_SIZEOF(ZCOMBUFR(1)))
!ov!!      CALL MPL_DOUBLESTACK%ALLOCATE(ZCOMBUFR,[ICOMBUFR_OFFSET(IRECV_COUNTS+1)],SIDE_PARAM=MPL_UPPER)
!ov!      CALL MPL_DOUBLESTACK%ALLOCATE(ZCOMBUFR,[ICOMBUFR_OFFSET(IRECV_COUNTS+1)],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
!ov!
!ov!    ENDIF
!ov!    IF (ISEND_COUNTS > 0) THEN
!ov!!      CALL ASSIGN_PTR(ZCOMBUFS, GET_ALLOCATION(ALLOCATOR, HTRLTOG%HCOMBUFR_AND_COMBUFS),&
!ov!!          & ALIGN(1_JPIB*KF_GP*D%NGPTOT*C_SIZEOF(ZCOMBUFR(1)),128)+1, &
!ov!!          & ICOMBUFS_OFFSET(ISEND_COUNTS+1)*C_SIZEOF(ZCOMBUFS(1)))
!ov!!      CALL MPL_DOUBLESTACK%ALLOCATE(ZCOMBUFS,[ICOMBUFS_OFFSET(ISEND_COUNTS+1)],SIDE_PARAM=MPL_UPPER)
!ov!
!ov!      CALL MPL_DOUBLESTACK%ALLOCATE(ZCOMBUFS,[ICOMBUFS_OFFSET(ISEND_COUNTS+1)],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
!ov!
!ov!    ENDIF
    ...
    ! Free this now
    DEALLOCATE(IFLDA)
    !order of deallocation is important for ZCOMBUFS / ZCOMBUFR

    IF (ASSOCIATED(ZCOMBUFS)) CALL MPL_DOUBLESTACK%DEALLOCATE(ZCOMBUFS)
    IF (ASSOCIATED(ZCOMBUFR)) CALL MPL_DOUBLESTACK%DEALLOCATE(ZCOMBUFR)
