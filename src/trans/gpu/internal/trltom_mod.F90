...
CONTAINS
  ...
  SUBROUTINE TRLTOM(ALLOCATOR,HTRLTOM,PFBUF_IN,PFBUF,KF_FS,LDCOMPUTE)
...
!        & 1_JPIB, 2_JPIB*D%NLENGT1B*KF_FS*C_SIZEOF(PFBUF(1)))
   ...
   CALL MPL_DOUBLESTACK%ALLOCATE(PFBUF,[2_JPIB*D%NLENGT1B*KF_FS],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
IF (LLCOMPUTE) THEN
#ifdef OMPGPU
