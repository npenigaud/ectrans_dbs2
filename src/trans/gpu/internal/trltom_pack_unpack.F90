...
CONTAINS
  ...
  SUBROUTINE TRLTOM_PACK(ALLOCATOR,HTRLTOM_PACK,PREEL_COMPLEX,FOUBUF_IN,KF_FS,LDCOMPUTE)
...
!        & 1_JPIB, 2_JPIB*D%NLENGT0B*KF_FS*C_SIZEOF(FOUBUF_IN(1)))
    ...
    CALL MPL_DOUBLESTACK%ALLOCATE(FOUBUF_IN,[2_JPIB*D%NLENGT0B*KF_FS],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
IF (LLCOMPUTE) THEN
#ifdef OMPGPU
...
ENDIF
  ...
  SUBROUTINE TRLTOM_UNPACK(ALLOCATOR,HTRLTOM_UNPACK,FOUBUF,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV,LDCOMPUTE)
    USE PARKIND_ECTRANS,             ONLY: JPIM, JPRBT, JPRD, JPIB
    USE TPM_DIM,                     ONLY: R
    USE TPM_GEOMETRY,                ONLY: G
    USE BUFFERED_ALLOCATOR_MOD,      ONLY: BUFFERED_ALLOCATOR, ASSIGN_PTR, GET_ALLOCATION
    ...
    IMPLICIT NONE
    LOGICAL, INTENT(IN), OPTIONAL :: LDCOMPUTE
    REAL(KIND=JPRBT), INTENT(IN), POINTER :: FOUBUF(:)
...
!!!    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPS,[IIN_SIZE],SIDE_PARAM=MPL_LOWER)
    CALL MPL_DOUBLESTACK%NEXT_SIDE()
    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPS,[IIN_SIZE],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
!    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPS,[IALLOC_SZ],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
...
!        & IALLOC_POS, IALLOC_SZ)
!    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPA,[IIN_SIZE],SIDE_PARAM=MPL_LOWER)
    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPA,[IIN_SIZE],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
!    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPA,[IALLOC_SZ],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
    IALLOC_POS=IALLOC_POS+IALLOC_SZ
...
!        & IALLOC_POS, IALLOC_SZ)
!    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPS0,[IIN0_SIZE],SIDE_PARAM=MPL_LOWER)
    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPS0,[IIN0_SIZE],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
!    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPS0,[IALLOC_SZ],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
    IALLOC_POS=IALLOC_POS+IALLOC_SZ
...
!        & IALLOC_POS, IALLOC_SZ)
!    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPA0,[IIN0_SIZE],SIDE_PARAM=MPL_LOWER)
    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPA0,[IIN0_SIZE],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
!    CALL MPL_DOUBLESTACK%ALLOCATE(ZINPA0,[IALLOC_SZ],SIDE_PARAM=MPL_DOUBLESTACK%CURRENT_SIDE,LD_ALLOC=LLCOMPUTE)
    IALLOC_POS=IALLOC_POS+IALLOC_SZ
IF (LLCOMPUTE) THEN
#ifdef OMPGPU
