...
CONTAINS
  ...
  SUBROUTINE TRLTOMAD_PACK(ALLOCATOR,HTRLTOM_PACK,PREEL_COMPLEX,FOUBUF_IN,KF_FS,LDCOMPUTE)
    ...
    REAL(KIND=JPRBT), INTENT(INOUT), POINTER :: FOUBUF_IN(:)
    INTEGER(KIND=JPIM),INTENT(IN) :: KF_FS
    TYPE(BUFFERED_ALLOCATOR), INTENT(IN) :: ALLOCATOR
    TYPE(TRLTOMAD_PACK_HANDLE), INTENT(IN) :: HTRLTOM_PACK

    INTEGER(KIND=JPIM) :: JM,JF,IGLG,OFFSET_VAR,KGL,J,N
...
!                  & 1_JPIB, 1_JPIB*KF_FS*D%NLENGTF*C_SIZEOF(PREEL_COMPLEX(1)))
    ...
    CALL MPL_DOUBLESTACK%ALLOCATE(PREEL_COMPLEX,[1_JPIB*KF_FS*D%NLENGTF])
IF (LLCOMPUTE) THEN
    N = 1_JPIB*KF_FS*D%NLENGTF

#ifdef ACCGPU
    !$ACC DATA PRESENT(PREEL_COMPLEX)
    !$ACC PARALLEL LOOP FIRSTPRIVATE(N)
#endif
#ifdef OMPGPU
    !$OMP TARGET DATA MAP(PRESENT,ALLOC:PREEL_COMPLEX)
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO DEFAULT(NONE) SHARED(N,PREEL_COMPLEX)
#endif
    DO J=1,N
      PREEL_COMPLEX(J) = 0
    ENDDO
...
ENDIF
  ...
  SUBROUTINE TRLTOMAD_UNPACK(ALLOCATOR,HTRLTOM_UNPACK,FOUBUF,ZINPS,ZINPA,ZINPS0,ZINPA0,KF_FS,KF_UV,LDCOMPUTE)
    USE PARKIND_ECTRANS,             ONLY: JPIM, JPRBT, JPRD, JPIB
    USE TPM_DIM,                     ONLY: R
    USE TPM_GEOMETRY,                ONLY: G
    USE BUFFERED_ALLOCATOR_MOD,      ONLY: BUFFERED_ALLOCATOR, ASSIGN_PTR, GET_ALLOCATION
    ...
    REAL(KIND=JPRBT), INTENT(IN), POINTER :: ZINPS(:), ZINPA(:)
    REAL(KIND=JPRD),  INTENT(IN), POINTER :: ZINPS0(:), ZINPA0(:)
...
!        & 1_JPIB, 2_JPIB*D%NLENGT1B*KF_FS*C_SIZEOF(FOUBUF(1)))
    CALL MPL_DOUBLESTACK%NEXT_SIDE()
    CALL MPL_DOUBLESTACK%ALLOCATE(FOUBUF,[2_JPIB*D%NLENGT1B*KF_FS])
    IF (LLCOMPUTE) THEN
#ifdef OMPGPU
