...
USE TPM_DISTR,                   ONLY: D, DISTR_RESOL, NPROC, MYPROC
...
IF (LLP2) WRITE(NOUT,'("ARRAY NLOEN ALLOCATED",8I8)')  SIZE(G%NLOEN   ),SHAPE(G%NLOEN   )
...
#endif
#ifdef OMPGPU
!  INUMDEVS = OMP_GET_NUM_DEVICES()
!  MYGPU = MOD(MYPROC-1, INUMDEVS)
!  CALL OMP_SET_DEFAULT_DEVICE(MYGPU)
#endif
