...
USE TPM_DISTR ,ONLY : D, NPROC, MYPROC, NPRCIDS, MTAGDISTGP
...
REAL(KIND=JPRM), POINTER, CONTIGUOUS :: ZBUF(:,:,:),ZRCV2(:,:)
REAL(KIND=JPRM), POINTER, CONTIGUOUS    :: ZRCV(:,:)
...
CALL MPL_DOUBLESTACK%ALLOCATE(ZBUF,[D%NGPTOTMX,IMYFIELDS,NPROC],SIDE_PARAM=MPL_UPPER)
CALL MPL_DOUBLESTACK%ALLOCATE(ZRCV,[D%NGPTOTMX,KFDISTG],SIDE_PARAM=MPL_UPPER)
...
!        ALLOCATE(ZRCV2(D%NGPTOTMX,IFLDSFROM(JROC)))
        CALL MPL_DOUBLESTACK%ALLOCATE(ZRCV2,[D%NGPTOTMX,IFLDSFROM(JROC)],SIDE_PARAM=MPL_LOWER)
        ...
        ENDDO
!        DEALLOCATE(ZRCV2)
        CALL MPL_DOUBLESTACK%DEALLOCATE(ZRCV2)
      ENDIF
    ENDDO
  ENDIF
...
!  IF(ALLOCATED(ZBUF)) DEALLOCATE(ZBUF)
  IF (ASSOCIATED(ZBUF)) CALL MPL_DOUBLESTACK%DEALLOCATE(ZBUF)
  IF (ASSOCIATED(ZRCV)) CALL MPL_DOUBLESTACK%DEALLOCATE(ZRCV)
ENDIF
